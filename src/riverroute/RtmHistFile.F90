...
!-----------------------------------------------------------------------
  subroutine RtmHistFileFinalize()
    ...
    integer :: t,f,fld  ! indices

    do t = 1,ntapes
       do f = 1, max_split_files
          do fld = 1, tape(t)%nflds(f)
             deallocate (tape(t)%hlist(fld,f)%hbuf)
             deallocate (tape(t)%hlist(fld,f)%nacs)
          end do
       end do
    end do
  ...
  subroutine htapes_fieldlist()
    ...
    tape_loop4: do t = 1, ntapes
       file_loop3: do f = 1, max_split_files
          if (tape(t)%nflds(f) == 0) then
             write(iulog,*) trim(subname),' ERROR: Tape ',t, f, ' is empty'
             call shr_sys_abort()
          end if
       end do file_loop3
    end do tape_loop4
...
!-----------------------------------------------------------------------
  ...
  subroutine htape_addfld (t, f, fld, avgflag)
    ...
    allocate (tape(t)%hlist(n,f)%hbuf(begrof:endrof))
    allocate (tape(t)%hlist(n,f)%nacs(begrof:endrof))
    ...
    if ( .not. lhistrest )then
       ...
       if (masterproc)then
          write(iulog,*) trim(subname), &
                          ' : Successfully defined netcdf history tape, file ', t, f
          call shr_sys_flush(iulog)
       end if
    else
       if (masterproc)then
          write(iulog,*) trim(subname), &
                          ' : Successfully defined netcdf restart history tape, file ', t, f
          call shr_sys_flush(iulog)
       end if
    end if
...
90     format(i2.2,':',i2.2,':',i2.2)
       ...
       if(writeTC .and. t == 1) then 
          call ncd_defvar(varname='fthresh', xtype=tape(t)%ncprec, dim1name='lon', &
               ...
               missing_value=spval, fill_value=spval, units='m3', ncid=nfid(t,f))
       endif
       ...
       call ncd_io(varname='lon', data=runoff%rlon, ncid=nfid(t,f), flag='write')
       call ncd_io(varname='lat', data=runoff%rlat, ncid=nfid(t,f), flag='write')

       if(writeTC .and. t == 1 .and. tape(t)%ntimes(f) == 1 ) then
          call ncd_io(varname='fthresh', data=runoff%fthresh, ncid=nfid(t,f), flag='write' &
               ,dim1name='allrof')
          writeTC = .false.
       endif
    endif
          ...
          if (tape(t)%is_endhist) then
             ...
             if (tape(t)%ntimes(f) == 1) then
                locfnh(t,f) = set_hist_filename (hist_freq=tape(t)%nhtfrq, &
                     rtmhist_mfilt=tape(t)%mfilt, hist_file=t, f_index=f)
                if (masterproc) then
    ...
    !================================================
       ...
       tape_loop1: do t = 1, ntapes
          file_loop1: do f = 1, max_split_files
             ...
             !
             write(hnum,'(i1.1)') t-1
             locfnhr(t,f) = "./" // trim(caseid) //".rtm"// trim(inst_suffix) &
       ...
       tape_loop4: do t = 1, ntapes
          file_loop4: do f = 1, max_split_files
             ...
             fld_loop5: do fld = 1, tape(t)%nflds(f)
                ...
                allocate (tape(t)%hlist(fld,f)%hbuf(begrof:endrof), &
                          tape(t)%hlist(fld,f)%nacs(begrof:endrof), stat=status)
                if (status /= 0) then
...
!-----------------------------------------------------------------------
  ...
  character(len=max_length_filename) function set_hist_filename (hist_freq, rtmhist_mfilt, hist_file, f_index)
